...
| UI | DOM + CSS overlay — dialogue, evidence, journal, deduction, menus, settings |
| Build | Vite |
| Content | JSON files under `/src/content` |
| Tests | Vitest |
...
- **GameState** (`src/engine/GameState.ts`) — the single serializable
  source of truth for a playthrough (flags, evidence, journal progress,
  deduction progress, scene/visual-state, active conversation). Pure data;
  no events, no logic beyond simple invariants (e.g. "don't add the same
  evidence twice").
- **conditions.ts** — evaluates `UnlockCondition[]` (flag / evidence /
...
|---|---|---|
| SceneManager | `engine/scene/SceneManager.ts` | GameState, EventBus, BackgroundManager, AudioManager, EvidenceSystem, InsightJournal, DeductionFramework, DialogueSystem |
| BackgroundManager | `engine/background/BackgroundManager.ts` | Phaser.Scene only |
| DialogueSystem | `engine/dialogue/DialogueSystem.ts` | GameState, EventBus, EvidenceSystem, InsightJournal |
...
| EvidenceSystem | `engine/evidence/EvidenceSystem.ts` | GameState, EventBus |
| InsightJournal | `engine/journal/InsightJournal.ts` | GameState, EventBus |
| DeductionFramework | `engine/deduction/DeductionFramework.ts` | GameState, EventBus, EvidenceSystem, InsightJournal |
...
All story data lives in `/src/content/*.json`, typed against the
interfaces in `src/engine/types.ts` (`SceneDefinition`,
`ConversationDefinition`, `EvidenceDefinition`, `JournalEntryDefinition`,
`DeductionDefinition`, `CharacterDefinition`). `src/content/index.ts` is
the single import point — adding a scene, conversation, or evidence item
means editing JSON there, never engine code. See `content-schema.md` for
field-by-field notes on each shape.
...
Every engine module (SceneManager, PortraitManager, EvidenceSystem, …)
